      ...
      <div className="studio-main">
        {!world ? <div className="studio-worlds" aria-label="Project workflow">
          ...
          {world === "audience" ? <div className="studio-detail-grid">
            ...
            </section><aside className="studio-side-note"><h3>Format suggestions</h3><p>Local starters based on uploaded material. Not AI-generated strategy or validated audience research.</p>
              <div className="studio-records">{studio?.suggestions.filter(item => item.channel === channel).slice(0, 5).map(item => <article key={item.id}><h4>{item.title}</h4><p>{item.detail}</p><button className="studio-pill" disabled={busy} onClick={() => { setTopic(item.title); setMaterialIds(item.material_ids); }}>Use suggestion</button></article>)}</div>
